...
/// The GG Circuit platform account this install is signed into. Tokens live in
...
fn default_platform_server_url() -> String {
    "https://api.gg.draggonlab.org".to_string()
}
...
/// `~/GG Circuit/Projects`, git-initialized by us) or was opened from an
...
mod tests {
        ...
        let settings = serde_json::from_value::<AppSettings>(json!({
            "workspace": {
                "workspaces": [
                    ...
                    {
                        "id": "managed",
                        "root": "/Users/example/GG Circuit/Projects/managed",
                        "kind": "managed",
    ...
    fn legacy_ai_provider_settings_are_ignored_silently() {
        ...
        assert_eq!(
            settings.platform.server_url,
            "https://api.gg.draggonlab.org"
        );
    ...
    fn platform_settings_normalize_server_url_and_blank_ids() {
        let settings = serde_json::from_value::<AppSettings>(json!({
            "platform": {
                "serverUrl": " https://staging.gg.draggonlab.org/ ",
                "accountId": "  ",
                "workspaceId": "wspc_1",
                "deviceId": ""
            }
        ...
        assert_eq!(
            settings.platform.server_url,
            "https://staging.gg.draggonlab.org"
        );
        assert!(settings.platform.account_id.is_none());
        ...
        .unwrap()
        .normalized();
        assert_eq!(blank.platform.server_url, "https://api.gg.draggonlab.org");
    }
