...
impl SbolObjectStorage {
    ...
    /// Graph membership comes from triples, not the object's latest-import
    /// graph_id, so objects shared by multiple imports remain visible in each.
            ...
            .prepare(
                ...
                WHERE is_deleted = 0
                  ...
                  AND (?6 IS NULL OR EXISTS (
                    ...
                    WHERE g.id = ?6 AND t.subject_iri = sbol_objects.iri
                  ))
                ORDER BY iri ASC
                LIMIT ?5
                "#,
    ...
    fn list_filters_iris_with_existing_object_filters() {
        ...
        connection
            .execute_batch(
                ...
                CREATE TABLE sbol_graphs (id TEXT NOT NULL, iri TEXT NOT NULL);
                CREATE TABLE sbol_triples (graph_iri TEXT NOT NULL, subject_iri TEXT NOT NULL);
                "#,
            )
