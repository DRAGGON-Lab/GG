//! Resolution and execution for GG Circuit's bundled relocatable CPython
...
///
/// `CARGO_MANIFEST_DIR` is `<workspace>/crates/gg-pyenv` at build time; the
...
pub const RUNNER_SOURCE: &str = include_str!("gg_runner.py");
...
pub const DISPLAY_SENTINEL: &str = "\u{1f}GG_DISPLAY\u{1f}";
