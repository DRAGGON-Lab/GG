...
//! python-lsp-server) for Bio Eng Studio's Python editor.
//!
        ...
        client
            .request(
                ...
                json!({
                    ...
                    "clientInfo": { "name": "Bio Eng Studio", "version": env!("CARGO_PKG_VERSION") },
...
mod tests {
    ...
    async fn lsp_roundtrip_hover_and_completion() {
        let Some(python) = bioeng_pyenv::python_executable(None) else {
        ...
        let uri = "file:///tmp/bioeng_lsp_test.py";
        let text = "import os\ndef greet(name):\n    return name\n\ngreet\nos.\n";
