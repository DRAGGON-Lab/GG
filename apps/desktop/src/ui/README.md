...
Bio Eng Studio uses Base UI as its headless interaction foundation. Product code should import from `src/ui` rather than directly from `@base-ui/react` so the app keeps a local design system boundary.

Keep this layer small and deliberate:

- Wrap primitives when Bio Eng Studio needs a stable visual or behavioral contract.
